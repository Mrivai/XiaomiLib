/*
 * ResourceFolderManager.cs - Developed by Mrivai for XiaomiLib.dll - 04/12/12
 */
...
{
    /// <summary>
    /// Controls Resource Folders for the <see cref="Mrivai"/> Namespace
    /// </summary>
    ...
    {
        private static readonly DirectoryInfo XIAOMI_TEMP_DIRECTORY;
        private static Dictionary<string, DirectoryInfo> controlledFolders;

        static ResourceFolderManager()
        {
            XIAOMI_TEMP_DIRECTORY = new DirectoryInfo(Path.GetTempPath() + "\\Mrivai\\");
            controlledFolders = new Dictionary<string, DirectoryInfo>();

            if (!XIAOMI_TEMP_DIRECTORY.Exists)
                XIAOMI_TEMP_DIRECTORY.Create();

            foreach (DirectoryInfo d in XIAOMI_TEMP_DIRECTORY.GetDirectories("*", SearchOption.TopDirectoryOnly))
        ...
        public static bool Register(string name)
        {
            ...
            controlledFolders.Add(name, new DirectoryInfo(XIAOMI_TEMP_DIRECTORY + name));
